...
class Sequence implements Comparable<Sequence>, Cloneable {
     ...
     * 判断序列中最后一个项集是否为单一的值
     */
    boolean isLastItemSetSingleNum(){
        ItemSet lastItemSet = getItemSetList().get(getItemSetList().size() - 1);
     ...
     * @param seq 比较的序列对象
     */
    boolean compareIsSame(Sequence seq){
        boolean result = true;
     ...
     * 生成此序列的所有子序列
     */
    ArrayList<Sequence> createChildSeqs(){

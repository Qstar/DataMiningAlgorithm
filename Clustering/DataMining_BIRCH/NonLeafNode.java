...
class NonLeafNode extends ClusteringFeature {
    ...
    ArrayList<NonLeafNode> getNonLeafChilds(){
        return nonLeafChilds;
    }

    LinkedList<LeafNode> getLeafChilds(){
        return leafChilds;
    }
     ...
     */
    private boolean addingNeededDivide(LeafNode leafNode){
     ...
     */
    private boolean addingNeededDivide(NonLeafNode nonLeafNode){
        boolean needDivided = false;
     ...
     * 因为叶子节点数超过阈值，进行分裂
     */
    private NonLeafNode[] leafNodeDivided(){
        ...
        for (LeafNode c : leafChilds) {
            if ((node1 != null ? node1.computerClusterDistance(c) : 0) < (node2 != null ? node2
                    .computerClusterDistance(c) : 0)) {
     ...
     * 因为非叶子节点数超过阈值，进行分裂
     */
    private NonLeafNode[] nonLeafNodeDivided(){
        NonLeafNode[] nonLeafNodes = new NonLeafNode[2];

        // 簇间距离差距最大的2个簇，后面的簇按照就近原则划分即可
        NonLeafNode node1 = null;
        NonLeafNode node2 = null;
        NonLeafNode tempNode;
        double maxValue = 0;
        double temp;
     ...
     * @param cluster 待添加聚簇
     */
    LeafNode findedClosestNode(Cluster cluster){
        LeafNode node = null;
        ...
        if (nonLeafChilds == null) {
            ...
            // 递归继续往下找
            node = nonLeafNode != null ? nonLeafNode.findedClosestNode(cluster) : null;
        }

        return node;
    }

    NonLeafNode getParentNode(){
        return parentNode;
    }

    private void setParentNode(NonLeafNode parentNode){

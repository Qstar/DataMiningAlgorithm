            ...
            if (y[j] == +1) {
                if (!is_lower_bound(j)) {
                    ...
                    if (grad_diff > 0) {
                        ...
                        double quad_coef = 0;
                        if (Q_i != null) {
                            quad_coef = Q_i[i] + QD[j] - 2.0 * y[i] * Q_i[j];
                        }
                        if (quad_coef > 0)
...
public class svm {
    ...
    private static svm_print_interface svm_print_string = System.out::print;
    ...
    private static void svm_cross_validation(svm_problem prob, svm_parameter param, int nr_fold, double[] target){
        int i;
    ...
    private static void svm_predict_values(svm_model model, svm_node[] x, double[] dec_values){
    ...
    private static double svm_predict_probability(svm_model model, svm_node[] x, double[] prob_estimates){
    ...
    public static void svm_save_model(String model_file_name, svm_model model) throws IOException{
        ...
        for (int i = 0; i < l; i++) {
            ...
            if (param.kernel_type == svm_parameter.PRECOMPUTED)
                ...
                for (svm_node aP : p) {
                    fp.writeBytes(aP.index + ":" + aP.value + " ");
                }
            fp.writeBytes("\n");
        }
